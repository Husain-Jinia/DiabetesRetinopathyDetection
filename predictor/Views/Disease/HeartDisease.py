...
from predictor.forms import DiabetesbasicPredictionForm, HeartDiseasePredForm
...
def heartdisease(request):
    ...
    form= HeartDiseasePredForm(request.POST )
    ...
    if form.is_valid():
        ...
        return redirect(f'/heartdiseaseresult/'+ str(forms_pk))
    return render(request,'heart/heartdiseaseform.html',context)


def heartdiseasepred(request,pk):
    pk=int(pk)

    values = HeartDisease.objects.get(pk=pk)
    ...
    loaded_model = pickle.load(open('/home/husain/Projects/IBM-mini-project/models/heart_modelBasic_pickle.sav', 'rb'))
...
def result_heartdisease_pdf(request,pk):
    buf = io.BytesIO() 
    fileName = 'HeartDiseaseResult.pdf'
    
    documentTitle = 'Heart Disease'
